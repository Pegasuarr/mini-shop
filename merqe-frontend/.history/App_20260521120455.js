...
function placeOrder() {
  ...
  // Create order with Order_Items structure
  const orderItems = cart.map(item => ({
    productId: item.productId,
    qty: item.qty,
    price: getProduct(item.productId).price,
...
function renderOrders() {
  ...
  ordersBody.innerHTML = DB.orders.map(order => {
    const itemsHtml = order.items.map(item => `
      <div class="order-item-row">
        <span>
          <i data-lucide="${getProductIcon(item.productId)}"></i>
...
function setupEventListeners() {
  ...
  if (learnMoreBtn) {
    learnMoreBtn.addEventListener("click", () => {
      showToast("✨ Discover our AI style matching! Upload your child's photo to see recommendations.");
    });
  }
  ...
  if (contactBtn) {
    contactBtn.addEventListener("click", () => {
      showToast("📧 Contact us: hello@minimerqe.com | 📞 +1 (888) 432-1234");
    });
  }
}
