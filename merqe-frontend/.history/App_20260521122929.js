...
async function fetchCategories() {
    try {
        ...
        showToast('Cannot connect to server. Make sure backend is running on port 8081', true);
    }
    return [];
}
...
async function placeOrder() {
    ...
    // FIXED: Use the correct format that backend expects
    // Based on typical Go backend structure with user info and items
    const orderData = {
        userId: 1,
        user: {
            name: name,
    ...
    if (result) {
        ...
        // Don't show another error if already shown in placeOrderBackend
        if (!result) {
            // Error already shown
        }
    }
        ...
        const itemsHtml = order.items && order.items.length > 0 ? order.items.map(item => {
            ...
            return `
                <div class="order-item-row">
                    <span>
                        <i data-lucide="${product?.icon || 'gift'}"></i>
                        ${item.productName || product?.name || `Product #${item.productId}`} × ${item.quantity}
                    </span>
...
async function init() {
    ...
    if (learnMoreBtn) {
        learnMoreBtn.addEventListener("click", () => {
            showToast("✨ Upload your child's photo for AI style matching!");
        });
    }
    ...
    if (products.length === 0 && categories.length === 0) {
        showToast("⚠️ Cannot connect to backend. Make sure it's running on port 8081", true);
    }
}
